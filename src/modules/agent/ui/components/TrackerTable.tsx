...
type AgentTrackerRow = TrackerRow & {
  project_id?: Id
  project_name?: string
  task_id?: Id
  task_name?: string
  ...
  useEffect(() => {
    ...
    const loadTrackers = async (): Promise<void> => {
      try {
        ...
        const responseData = okStatus && asRecord(res.data) ? res.data.data : undefined
        ...
        const enriched = fetched
          .filter((tracker) => {
            ...
            if (selectedProject && String(tracker.project_id) !== String(selectedProject))
              return false
            if (selectedTask && String(tracker.task_id) !== String(selectedTask))
              return false
    ...
    void loadTrackers()
  }, [userId, startDate, endDate, selectedProject, selectedTask, getProjectName, getTaskName])
          ...
          <div className="bg-white rounded-lg shadow-lg p-8 max-w-sm w-full pointer-events-auto">
            ...
            <div className="flex justify-end gap-4">
              ...
              <button
                ...
                disabled={deletingId != null}
              >{deletingId != null ? "Deleting..." : "Delete"}</button>
            </div>
          </div>
        </div>
      )}
