...
const BillableReport: React.FC = () => {
	...
	const [monthlySummaryData, setMonthlySummaryData] = useState<MonthlyBillableReportRow[]>([]);
	...
	const groupByMonthYear = (data: MonthlyBillableReportRow[]): Record<string, MonthlyBillableReportRow[]> => {
	...
	const handleExportUserDaily = async (userObj: UserObj) => {
		try {
			...
			const res = await fetchDailyBillableReport(payload);
			const trackers: TrackerRow[] = Array.isArray(res.data?.trackers) ? res.data.trackers : [];
			...
			exportData.push({
				'Date-Time': 'TOTAL',
				'Assign Hours': '-',
				'Worked Hours': totalWorked.toString(),
				'QC score': avgQC,
				'Daily Required Hours': totalRequired.toString(),
			});
	...
	const handleExportMonthData = async (monthObj: MonthObj, users: MonthlyBillableReportRow[]) => {
		try {
							...
							{!isAgent ? (
								...
								<UserCard
									user={{
										...
										team_name: typeof user?.team_name === 'string' ? user.team_name : ''
									}}
