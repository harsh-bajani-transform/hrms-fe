...
export default function UserCard({ user, dailyData, defaultCollapsed, formatDateTime: customFormatDateTime }) {
  const { user: currentUser } = useAuth();
  const isAgent = currentUser?.role_id === 6 || currentUser?.role_name === 'agent';
  ...
  // Use custom formatDateTime if provided, otherwise use local one
  const formatFn = customFormatDateTime || formatDateTime;
          ...
          <tbody>
            {filteredRows.map((row, idx) => (
              <tr key={row.date_time || row.date || idx} className="hover:bg-blue-50 transition group">
                <td className="px-4 py-3 text-black font-medium whitespace-nowrap">{formatFn(row.date_time || row.date)}</td>
