...
const QAAgentList: React.FC = () => {
	...
	const [dropdownTaskNameMap, setDropdownTaskNameMap] = useState<DropdownTaskNameMap>({});
	...
	useEffect(() => {
		const fetchDashboardData = async () => {
			try {
				...
				const projectsWithTasks: ProjectWithTasks[] = dropdownRes.data?.data || [];
				...
				let trackersByAgent: AgentTrackersMap = {};
				const role = String(user?.role_name || user?.user_role || '').toLowerCase();
				const allUsers: Agent[] = data.users || [];
				const allTrackers: Tracker[] = data.tracker || [];

				if (role === 'assistant manager' || role.includes('assistant')) {
					let myTeamIds: string[] = [];
					if (data.projects && user && user.user_id) {
						data.projects.forEach((p: { asst_project_manager_id?: string; project_team_id?: string }) => {
					...
					if (data.projects && user && user.user_id) {
						data.projects.forEach((p: { project_manager_id?: string | number; project_team_id?: string }) => {
					...
					let myQAIds: string[] = [];
					if (data.projects && user && user.user_id) {
						data.projects.forEach((p: { project_qa_id?: string; project_team_id?: string }) => {
															...
															<tbody>
																{trackers.map((tracker, index) => (
																	...
																	>
																		...
																		<td className="px-5 py-3 text-slate-700 whitespace-nowrap">
																			{tracker.task_name || dropdownTaskNameMap[String(tracker.task_id)] || "-"}
																		</td>
