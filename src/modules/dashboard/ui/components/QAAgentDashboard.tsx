  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        setLoading(true);
        log(
          "[QAAgentDashboard] Fetching dashboard data with dates:",
          dateRange,
        );
        ...
        if (res.status === 200 && res.data?.data) {
          ...
          const trackersWithFiles: TrackerFile[] = trackers
            ...
            .map((tracker) => {
              const taskInfo = taskMap[String(tracker.task_id)] ?? {
                task_name: "",
                task_target: null,
              };
              return {
          ...
          log(
            "[QAAgentDashboard] Dashboard data loaded - Agents:",
            users.length,
            "Files:",
            trackersWithFiles.length,
          );
        }
      } catch (err) {
              ...
              <div className="flex items-center gap-3 text-white">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-100 mt-0.5">
                    Files awaiting quality check review
                  </p>
                </div>
              </div>
            </div>
