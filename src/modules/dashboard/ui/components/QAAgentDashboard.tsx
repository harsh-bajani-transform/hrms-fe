...
import QATabsNavigation, { QATabId } from "../../../qa/ui/components/QATabsNavigation";
...
const QAAgentDashboard: React.FC<QAAgentDashboardProps> = ({ dateRange }) => {
	...
	const [pendingFiles, setPendingFiles] = useState<TrackerFile[]>([]);
		...
		const fetchDashboardData = async () => {
			try {
				...
				if (res.status === 200 && res.data?.data) {
					...
					const trackers: TrackerFile[] = responseData.tracker || [];
					const users: { id: number | string }[] = responseData.users || [];
					const tasks: { task_id: number | string; task_name: string; task_target: string | number | null }[] = responseData.tasks || [];
					const summary = responseData.summary || {};

					const taskMap: Record<string, TaskMapValue> = {};
					tasks.forEach((task) => {
					...
					const trackersWithFiles: TrackerFile[] = trackers
						.filter((tracker) => tracker.tracker_file)
						.map((tracker) => {
							const taskInfo = taskMap[String(tracker.task_id)] ?? { task_name: '', task_target: null };
							return {
								...tracker,
	...
	const StatCard: React.FC<StatCardProps> = ({ title, value, subtitle, icon: Icon, iconBgColor, iconColor }) => (
