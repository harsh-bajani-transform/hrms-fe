...
import TaskFormModal, { TaskType } from './TaskFormModal';
...
const ProjectsManagement: React.FC<ProjectsManagementProps> = ({ projects, loading, onRefresh, dropdowns }) => {
  ...
  const handleDeleteTask = async (projectId: string | number, task: TaskType) => {
  ...
  const openTaskModal = (project: ProjectType, task: TaskType | null = null) => {
    setTaskModalState({
      isOpen: true,
      project,
      task,
                        ...
                        ? p.tasks.map((t) => (
                            ...
                            >
                              <div className="flex-1 mr-2 cursor-pointer" onClick={() => canManageProjects && openTaskModal(p, t)}>
                                ...
                                {typeof t.task_target === 'string' || typeof t.task_target === 'number' ? (
        ...
        <TaskFormModal
          ...
          dropdowns={{
            users: dropdowns.agents.map(a => ({
              ...a,
              label: typeof a.label === 'string' ? a.label : String(a.user_name || a.id || a.user_id || ''),
            })),
