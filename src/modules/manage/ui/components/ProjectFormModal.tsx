...
interface ProjectType {
  ...
  qa_id?: string | number;
  monthly_hours_target?: number | string;
  [key: string]: unknown;
}
...
const ProjectFormModal: React.FC<ProjectFormModalProps> = ({ project, onClose, onSuccess, dropdowns }) => {
  ...
  const validate = () => {
    ...
    if (!formData.monthlyTarget || isNaN(Number(formData.monthlyTarget))) newErrors.monthlyTarget = 'Monthly target is required and must be a number';
    setErrors(newErrors);
        ...
        <div className="px-6 py-4 bg-indigo-600 text-white flex justify-between items-center">
          <div>
            ...
            <p className="text-indigo-100 text-xs mt-0.5">
              {isEditMode ? `Updating ${project?.project_name}` : 'Fill in the information to create a new project'}
            </p>
          </div>
          ...
          <form id="project-form" onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-700 ml-1">Owner</label>
                  <CustomSelect
                    value={formData.ownerId}
                    onChange={(val: string) => setFormData({ ...formData, ownerId: val })}
                    options={dropdowns.projectManagers.map(m => ({ value: (m.user_id || m.id)?.toString() || '', label: typeof m.label === 'string' ? m.label : String(m.label ?? '') }))}
                    placeholder="Select Owner"
                    className={errors.ownerId ? 'border-rose-400' : ''}
                  />
                  {errors.ownerId && <p className="text-[10px] text-rose-500 font-bold ml-1">{errors.ownerId}</p>}
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-700 ml-1">Assistant Manager</label>
                  <CustomSelect
                    value={formData.apmId}
                    onChange={(val: string) => setFormData({ ...formData, apmId: val })}
                    options={dropdowns.assistantManagers.map(m => ({ value: (m.user_id || m.id)?.toString() || '', label: typeof m.label === 'string' ? m.label : String(m.label ?? '') }))}
                    placeholder="Select AM"
                    className={errors.apmId ? 'border-rose-400' : ''}
                  />
                  {errors.apmId && <p className="text-[10px] text-rose-500 font-bold ml-1">{errors.apmId}</p>}
                </div>
              </div>
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-700 ml-1">QA Owner</label>
                <CustomSelect
                  value={formData.qaId}
                  onChange={(val: string) => setFormData({ ...formData, qaId: val })}
                  options={dropdowns.qas.map(q => ({ value: (q.user_id || q.id)?.toString() || '', label: typeof q.label === 'string' ? q.label : String(q.label ?? '') }))}
                  placeholder="Select QA"
                  className={errors.qaId ? 'border-rose-400' : ''}
                />
                {errors.qaId && <p className="text-[10px] text-rose-500 font-bold ml-1">{errors.qaId}</p>}
              </div>
              <div className="space-y-1">
                ...
                />
                {errors.monthlyTarget && <p className="text-[10px] text-rose-500 font-bold ml-1">{errors.monthlyTarget}</p>}
              </div>
            </div>
          </form>
        </div>
