  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      try {
        ...
        const projectsWithTasks: ProjectRef[] = Array.isArray(dropdownRes?.data) ? dropdownRes.data : [];
        const taskNameMap: TaskNameMap = {};
        projectsWithTasks.forEach((project) => {
          (project.tasks || []).forEach((task) => {
            if (task.task_id !== undefined) {
              taskNameMap[String(task.task_id)] = task.task_name || task.label || '';
            }
          });
        });
        ...
        if (role === "assistant manager") {
            ...
            data.projects.forEach((p: ProjectRef) => {
              if (p.asst_project_manager_id && user && p.asst_project_manager_id.includes(String(user.user_id))) {
                if (p.project_team_id) {
                  const ids = p.project_team_id.replace(/\[|\]/g, "").split(",").map((x: string) => x.trim()).filter(Boolean);
                  myTeamIds.push(...ids);
                }
              }
            });
          }
          filteredAgents = allUsers.filter((u) => u.user_id !== undefined && myTeamIds.includes(String(u.user_id)));
          ...
          filteredAgents = allUsers.filter((u) => u.user_id !== undefined && myProjectIds.includes(String(u.user_id)));
          ...
          filteredAgents = allUsers.filter((u) => u.user_id !== undefined && myQAIds.includes(String(u.user_id)));
        } else {
          filteredAgents = allUsers;
        }
        filteredAgents.forEach((agent) => {
          if (agent.user_id !== undefined) {
            trackersByAgent[String(agent.user_id)] = allTrackers.filter((t) => String(t.user_id) === String(agent.user_id) && t.tracker_file);
          }
        });
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {agents.map((agent) => {
            const agentId = agent.user_id !== undefined ? String(agent.user_id) : undefined;
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center">
                      <span className="text-blue-700 font-bold text-sm">
                        {agent.user_name ? agent.user_name.charAt(0).toUpperCase() : "-"}
                      </span>
                    </div>
                    <div>
                    ...
                    {trackers.length === 0 ? (
                      ...
                      <div className="overflow-x-auto">
                        <table className="min-w-full text-sm">
                          ...
                          <tbody>
                            {trackers.map((tracker: TrackerRow, index: number) => (
                              <tr
                              ...
                              >
                                ...
                                <td className="px-4 py-3 text-slate-700">
                                  {tracker.task_name || (tracker.task_id !== undefined ? dropdownTaskNameMap[String(tracker.task_id)] : undefined) || "-"}
                                </td>
